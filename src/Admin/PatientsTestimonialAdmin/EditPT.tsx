...
import { useEditPTMutation, useGetPTByIdQuery } from '../../API/PTApi';
...
function EditPT() {
    ...
    const{data, isLoading}=useGetPTByIdQuery(id);
    const[editPT]=useEditPTMutation();
    ...
    const handleSubmit=async(e:React.FormEvent<HTMLFormElement>)=>{
      ...
      const response:ApiResponseInterface=await editPT({
        id:id,
        PTData:formData
      });
      if(response.data?.isSuccess==true){
        setLoading(false);
        Toastify("Patients Testimonial updated successfully");
        navigate("/Admin/PatientsTestimonialAdmin/PatientsTestimonialAdminList");
        // @ts-ignore
  ...
  return (
    <div className='container'>
        <h1 className='text-success'>Edit Patients Testimonial</h1>
        <form method='post' encType='multipart/form-data' onSubmit={(e)=>handleSubmit(e)}>
            ...
            {/* @ts-ignore */}
            <button onClick={()=>{navigate("/Admin/PatientsTestimonialAdmin/PatientsTestimonialAdminList");window.scrollTo({top:0, left:0, behavior:"instant"})}} className='btn btn-secondary btn-block mx-2' style={{width:"100%", maxWidth:"199px"}}>Back</button>
            </div>
        </form>
    </div>
  )
